...
class TeleVisionWrapper:
    def __init__(self, head, lw, rw):
        self.head_matrix = head
        self.left_hand = lw
        self.right_hand = rw
        # self.left_landmarks = ll
        # self.right_landmarks = rl
    ...
    def get_data(self):
        ...
        #    z0 z1 z2 ··· z23 z24
        #     1  1  1 ···   1   1
        # left_hand_vuer_mat  = np.concatenate([self.left_landmarks.copy().T, np.ones((1, self.left_landmarks.shape[0]))])
        # right_hand_vuer_mat = np.concatenate([self.right_landmarks.copy().T, np.ones((1, self.right_landmarks.shape[0]))])
