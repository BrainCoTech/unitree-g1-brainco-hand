...
强脑灵巧手Revo2适配宇树G1(Edu标准版23/进阶版29自由度)教程及简单动作示例。
...
2. 等待（约1分钟）宇树G1进入**零力矩模式(L2 + Y)**，具体表现为随意活动关节无阻力。
3. 使用遥控器，按照说明按下对应按钮，使机器人依次进入**阻尼模式(L2 + B)** → **锁定站立模式(L2 + UP)**。
4. 可根据需要进入**常规运控模式(R1 + X)**，上肢低层运控与走跑高层运控不冲突。
