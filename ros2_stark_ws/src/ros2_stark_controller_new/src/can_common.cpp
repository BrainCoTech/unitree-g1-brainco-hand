...
static bool load_zlg_library(void) {
  ...
  if (lib_path && lib_path[0]) {
    g_zlg_lib = LoadLibrary(lib_path);
  }
  if (!g_zlg_lib) g_zlg_lib = LoadLibrary("libusbcanfd.dll");
  if (!g_zlg_lib) g_zlg_lib = LoadLibrary("usbcanfd.dll");
#else
  if (lib_path && lib_path[0]) {
...
#if PLATFORM_WINDOWS
  #define LOAD_SYM(name) p##name = (name##_t)GetProcAddress((HMODULE)g_zlg_lib, #name)
#else
