...
class G1_29_ArmController:
    def __init__(self, motion_mode = False, simulation_mode = False):
        ...
        #     time.sleep(0.1)
        #     logger.warning("[G1_29_ArmController] Waiting to subscribe dds...")
        ...
        # logger.debug(f"Current all body motor state q:\n{self.all_motor_q} \n")
        # logger.debug(f"Current two arms motor state q:\n{self.get_current_dual_arm_q()}\n")
...
class H1_2_ArmController:
    def __init__(self, motion_mode = False, simulation_mode = False):
        ...
        logger.info("Initialize H1_2_ArmController...")
        self.q_target = np.zeros(14)
        self.tauff_target = np.zeros(14)
        ...
        while not self.lowstate_buffer.GetData():
            time.sleep(0.1)
            logger.warning("[H1_2_ArmController] Waiting to subscribe dds...")
        logger.info("[H1_2_ArmController] Subscribe dds ok.")
        ...
        logger.info("Initialize H1_2_ArmController OK!")
    ...
    def ctrl_dual_arm_go_home(self):
        '''Move both the left and right arms of the robot to their home position by setting the target joint angles (q) and torques (tau) to zero.'''
        logger.info("[H1_2_ArmController] ctrl_dual_arm_go_home start...")
        max_attempts = 100
        current_attempts = 0
        with self.ctrl_lock:
            self.q_target = np.zeros(14)
            ...
            if np.all(np.abs(current_q) < tolerance):
                if self.motion_mode:
                    for weight in np.linspace(1, 0, num=101):
                        ...
                        time.sleep(0.02)
                logger.info("[H1_2_ArmController] both arms have reached the home position.")
                break
            current_attempts += 1
            time.sleep(0.05)
...
class H1_ArmController:
    def __init__(self, simulation_mode = False):
        ...
        logger.info("Initialize H1_ArmController...")
        ...
        while not self.lowstate_buffer.GetData():
            time.sleep(0.1)
            logger.warning("[H1_ArmController] Waiting to subscribe dds...")
        logger.info("[H1_ArmController] Subscribe dds ok.")
        ...
        logger.info("Initialize H1_ArmController OK!")
    ...
    def ctrl_dual_arm_go_home(self):
        '''Move both the left and right arms of the robot to their home position by setting the target joint angles (q) and torques (tau) to zero.'''
        logger.info("[H1_ArmController] ctrl_dual_arm_go_home start...")
        max_attempts = 100
        current_attempts = 0
        with self.ctrl_lock:
            self.q_target = np.zeros(8)
        ...
        while current_attempts < max_attempts:
            current_q = self.get_current_dual_arm_q()
            if np.all(np.abs(current_q) < tolerance):
                logger.info("[H1_ArmController] both arms have reached the home position.")
                break
            current_attempts += 1
            time.sleep(0.05)
