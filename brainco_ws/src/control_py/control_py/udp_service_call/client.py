...
async def main():
    robots = await discover(1.2)

    print("Found robots:")
    for r in robots:
        print(f" - {r['id']} {r['name']} @ {r['host']}:{r['tcp_port']} (hash={r['commands_hash']})")
    ...
    r = robots[0]
    host, port = r["host"], int(r["tcp_port"])

    # 使用 RobotClient 进行网络交互
    async with RobotClient(host, port) as client:
        # 获取命令列表
        ...
        # 执行命令样例
        if commands:
            # 执行第一个命令作为演示（通常是 enable）
            spec = next((c for c in commands if c.name == "enable"), commands[0])
            print(f"\n--- Executing command: {spec.name} ---")
            print(f"Description: {spec.desc}")
            print(f"Args schema: {json_dumps(spec.args or {})}")
            
            resp = await client.exec_command(spec.name)
            print(f"Response: {json_dumps(resp)}")
